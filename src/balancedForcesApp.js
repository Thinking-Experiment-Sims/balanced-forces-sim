...
class BalancedForcesApp {
  constructor() {
    ...
    // State
    this.state = {
      activeScenario: 'symmetric', // 'symmetric' | 'asymmetric' | 'mystery' | 'sandbox'
      workflowStep: 'measure',    // 'measure' | 'vector'
      massKg: 0.500,               // 500g default
      g: GRAVITY_STANDARD,

      // Apparatus geometry in virtual units [0..900, 0..520]
      s1: { x: 160, y: 120 },
      s2: { x: 740, y: 120 },
      p: { x: 450, y: 310 },

      // Interactive Protractor Tool
      protractor: {
        visible: true,
        x: 450,
        y: 310,
  ...
  resizeCanvases() {
    ...
    canvases.forEach(canvas => {
      ...
      const w = Math.max(280, Math.floor(rect.width || canvas.clientWidth || 900));
      const h = Math.max(200, Math.floor(rect.height || canvas.clientHeight || 520));
  ...
  updateEquilibrium() {
    ...
    this.equilibrium = calculateStaticEquilibrium(
      massKg,
      this.state.p,
      this.state.s1,
      this.state.s2,
  ...
  setScenario(scenarioId) {
    ...
    if (scenarioId === 'symmetric') {
      ...
      this.dom.bannerDesc.textContent = 'The hanging mass is unknown. Measure cord tensions and angles to calculate the load and verify equilibrium.';
      ...
      this.state.showAngles = false; // encourage manual measuring with protractor
  ...
  handleCheckMystery() {
    ...
    const evalReport = evaluateStudentCalculation(studentG, actualG);
  ...
  solveWorkbench() {
    ...
    const calc = calculateMassFromMeasurements(t1, t2, th1, th2, this.state.g);
    const actualKg = this.getActiveMassKg();
    const actualG = actualKg * 1000;
    const pErr = calculatePercentError(calc.calculatedMassG, actualG);
  ...
  recordTrial() {
    ...
    const calc = calculateMassFromMeasurements(
      eq.t1,
      eq.t2,
  ...
  bindCanvasMouse() {
    ...
    const handleDown = (e) => {
      ...
      if (this.state.protractor.visible) {
        ...
        if (Math.hypot(pos.x - rotHandleX, pos.y - rotHandleY) < 18) {
          this.state.dragTarget = 'protractor_rot';
          e.preventDefault();
          return;
        }

        // Check Protractor Body
    ...
    const handleMove = (e) => {
      ...
      if (!this.state.dragTarget) {
        const distToKnot = Math.hypot(pos.x - this.state.p.x, pos.y - this.state.p.y);
        this.state.isHoveringKnot = distToKnot < 22;
      ...
      if (this.state.dragTarget === 'knot') {
        ...
        if (Math.hypot(this.state.protractor.x - this.state.p.x, this.state.protractor.y - this.state.p.y) < 14) {
          this.state.protractor.x = this.state.p.x;
          this.state.protractor.y = this.state.p.y;
          this.state.protractor.isSnapped = true;
        }
  ...
  renderApparatus() {
    ...
    // Coordinate scale factor from virtual (900x520) to actual canvas
    ...
    // 1. Blueprint Grid & Lab Surface
    this.drawLabGrid(ctx);

    // 2. Dual Lab Ring Stands
